...
void init_matrix_random(double *matrix1d, int rows, int cols, FILE *fp)
{
    int ni, mod_rand=10;
    for (ni=0; ni<(rows*cols); ni++)
    {
...
void init_matrix_increment(double *matrix1d, int rows, int cols, FILE *fp)
{
    int ni;    
    for (ni=0; ni<(rows*cols); ni++)
    {
...
void init_matrix_zero(double *matrix1d, int rows, int cols, FILE *fp)
{
    int ni;
    for (ni=0; ni<(rows*cols); ni++)
    {
...
//  data file name .dat
    ...
    fprintf(fp_matrix, "# Program : \tA3-omp-1D \n# where :\t.dat contains timing data & .txt contains matrix values \n# \n");
    fprintf(fp_matrix, "# Summary of values added to each matrix - retained for later reference and validation \n# \n");
    fprintf(fp_matrix, "#  --------------------------------------------------------------------------------------------------  \n ");

    fprintf(fp_timing, "#  --------------------------------------------------------------------------------------------------\n# \n");
    fprintf(fp_timing, "# RUNNING : \t%s %s %d %d %s %s \n", argv[0], init_type, nx, nt, filename_matrix, filename_timing );
    fprintf(fp_timing, "# Program : \tA3-omp-1D \n# where :\t.dat contains timing data & .txt contains matrix values \n");
    fprintf(fp_timing, "#  --------------------------------------------------------------------------------------------------\n");
    fprintf(fp_timing, "# Matrix Size \tTime/manual \tInfinity Norm/manual \tTime/dgemm \tInfinity Norm/dgemm \n# \n");

    fprintf(stdout, "\n# RUNNING : \t%s %s %d %d %s %s \n", argv[0], init_type, nx, nt, filename_matrix, filename_timing);

//  CREATE & INITIALIZE :: matrices A & B & C
    ...
    {
        init_matrix_random(A, nx, ny, fp_matrix);
        init_matrix_random(B, nx, ny, fp_matrix);
    } else if (increment_or_random == 1) 
    {
        init_matrix_increment(A, nx, ny, fp_matrix);
        init_matrix_increment(B, nx, ny, fp_matrix);
    }
...
//  CALCULATION :: |C| and infinity norm
...
//  OUTPUT :: results to stdout & .dat file : matrix size || infinity norm || Time/manual || infinity norm || Time / dgemm
    fprintf(stdout,"\nMatrix Size \tTime/manual \tInfinity Norm/manual \tTime/dgemm \tInfinity Norm/dgemm\n");
    fprintf(stdout,"%d \t \t%lfs \t%g \t%lfs \t%g \n", nx, elapsed_manual, norm_manual, elapsed_dgemm, norm_dgemm);
