...
* *******************************************************************************
    ...
    The program A4-mpi-manual.c computes the time taked for manual evaulation of |allC| and 
...
void initialize_data_file_contents (FILE *fp) 
{
    ...
    fprintf(fp, "# Program : \tA4-mpi-solo \n# where : \t.dat contains timing data & .txt contains matrix values\n# \n");
    fprintf(fp, "# --------------------------------------------------------------------------------------------------\n# \n");
    fprintf(fp, "# |Matrix| \t|Processors| \tTime/mpi \tInf Norm/mpi \tTime/manual \tInf Norm/manual\n# \n");
}
...
//  validate number of processors 
    if ((nx % np) != 0)
    {
        fprintf(stderr, "\nWARNING : \t<np> [%d] : number of processors must divide evenly into <nx> [%d] : matrix size. Exiting. \n", np, nx);
        MPI_Finalize();  
        usage(); 
    }
    if (np > nx)
...
//  ALLOCATE & INITIALIZE :: matrices and output results to matrix file for reference
    ...
    {
            fprintf(fp_matrix, "# \t\t<%d> x <%d> matrix |segmentA| using incremental <segment column> value + 1... \n", my_nx, ny);
            initialize_matrix_increment(segmentA, my_nx, ny);
            print_matrix(segmentA, my_nx, ny, fp_matrix);
            fprintf(fp_matrix, "# \t\t<%d> x <%d> matrix |segmentB| using incremental <segment column> value + 1 ... \n", my_nx, ny);
    ...
    MPI_Barrier(MPI_COMM_WORLD);
//  MPI execution : calculate |allC| and infinity norm for resulting |allC|
    if (rank == 0)  
    {
...
//  OUTPUT :: results to stdout & .dat file
        fprintf(stdout,"# SUMMARY:\t|Matrix|  |Processors|  Time/mpi Inf Norm/mpi  Time/manual Inf Norm/manual\n");
        fprintf(stdout,"\t\t%d \t%d \t%f \t%.1f \t%f \t%.1f \n", nx, np, mpi_elapsed, mpi_norm, manual_elapsed, manual_norm);
