...
DatSuffix=".dat"
pngSuffix=".png"
stdLogFile="${logDir}/runAssignment3-${Now}.log"
...
usage() 
{
    $_ECHO -e "\nUSAGE :\t./$($_BASENAME $0) -d2|--cblas -p|--plot -r|--random -i|--increment -m|--matrix <n> -v|--values -?|-h|--help \n"
    $_ECHO -e "TO :\tCalculate |C| = |A| x |B| and then infintiy norm using Open MP." 
    $_ECHO -e "LOGS :\tCreated in <${logDir}> : <file>.txt matrix values for matrices |A| |B| & |C|, \n\t<file>.dat : timing of each computation & <${logDir}/runAssignment3-timestamp.log> summary of stdout. \n"
    $_ECHO -e "WHERE :\t-d2|--cblas\tCompile .c source files using dgemm cblas &  openmp"
    $_ECHO -e "\t-p|--plot\tPlot graphs using GnuPlot creating .png for each algorithm and store in <${graphDir}> for time taken-v- infinity norm \n"
...
error() 
{
    ...
    case ${err} in
        1)
            $_ECHO -e "ERROR ${err} :\t$($_BASENAME $0): Unknown parameter '${1}'." >> -a ${stdLogFile}
            ;;
        2)
            $_ECHO -e "ERROR ${err} :\t$($_BASENAME $0): Error creating directory '${1}'." >> -a ${stdLogFile}
            ;;
        3)
            $_ECHO -e "ERROR ${err} :\t$($_BASENAME $0): Error creating file '${1}'." >> -a ${stdLogFile}
            ;;     
        4)     
            $_ECHO -e "ERROR ${err} :\t$($_BASENAME $0): Missing parameter for '${1}'." >> -a ${stdLogFile}
            ;;     
        5)     
            $_ECHO -e "ERROR ${err} :\t$($_BASENAME $0): '${1}', Values entered are not valid or not a number." >> -a ${stdLogFile}
            ;;     
        6)     
            $_ECHO -e "ERROR ${err} :\t$($_BASENAME $0): '${1}', Mutually exclusive switches." >>  -a ${stdLogFile}
            ;;     
        7)     
            $_ECHO -e "ERROR ${err} :\t$($_BASENAME $0): '${1}', Compilation failed." >>  -a ${stdLogFile}
            ;;     
        8)     
            $_ECHO -e "ERROR ${err} :\t$($_BASENAME $0): '${1}', GnuPlot - graph export error." >>  -a ${stdLogFile}
            ;;     
        *)
            $_ECHO -e "ERROR :\tUnknown error." >> -a ${stdLogFile}
            $_ECHO ${err} >> -a ${stdLogFile}
            $_ECHO $* >> -a ${stdLogFile}
            ;;
    esac
    $_ECHO -e
...
plot_graph()
{
    ...
    localPngGraph=$2
    $_ECHO -e "RUNNING :\tCreating <localPngGraph>=${localPngGraph} in <graph Dir>=${graphDir} from <localDatFileToGraph>=${localDatFileToGraph}" >> ${localLogFile}
}
...
init_dir() 
{
    ...
    if [ ! -d ${creationDir} ] || [ ! -e ${creationDir} ] ; then 
        mkdir ${creationDir}        
        $_ECHO -e "WARNING :\tCreating $creationDir" >> -a ${stdLogFile}
        if [[ $? -ne 0 ]] ; then 
            error 2 $creationDir
        fi
    fi  
...
algorithm_execute() 
{
    ...
    localFileMatrix="$3"
    localFileTime="$4"
    $_ECHO -e "RUNNING :\t${localCmd} ${localOptions} ${localFileMatrix} ${localFileTime}" >> -a ${stdLogFile}
...
if  [ "${defaultMatrixRange}" == "true" ] && [ "${matrixEnabled}" == "false" ] ; then 
    declare -a NXArray=( 50 50 50 100 100 100 500 500 500 500 1000 1000 1000 1000 )
  	declare -a threadArray=( 10 10 10 10 10 10 20 20 20 20 20 20 20 20 )
    matrixEnabled="false"
    echo ${#NXArray[@]} 
else
    if [ "${matrixEnabled}" == "true" ] ; then
		...
		if [ ${matrixSize} -le 0 ] || [ ${matrixSize} -gt ${maxMatrixSize} ] ; then
		    $_ECHO -e "WARNING :\t$($_BASENAME $0): matrix size <nx> is invalid, now set to default of : $maxMatrixSize" >> -a ${stdLogFile}
...
if [[ ${#NXArray[*]} -gt 0 ]] ; then 
	...
	for (( i = 0 ; i < ${#NXArray[@]} ; i++ )); do
	    ...
	    executeOptions="${algorithmOptions} ${NXArray[$i]} 10"
