...
init_log_file() 
{
    ...
    echo -e "# file name : $LOG_DIR/${l_LOG_FILE} \n containing: \tmatrix valuesfor |C| += |A| x |B| \n# \n# "> $LOG_DIR/${l_LOG_FILE}
}
...
if [ "$BUILD_SIMPLE" == "true" ]  || [ "$BUILD_ALL" == "true" ] ; then
    ...
    if [[ ${#NX_ARRAY[*]} -ne ${#NB_ARRAY[*]} ]] ; then 
        ...
        for (( i = 0 ; i < ${#NX_ARRAY[@]} ; i++ )) do
            ...
            add_comments_to_log_file $LOG_FILE_SIMPLE_VALUES $ALGORITHIM_SIMPLE $BLOCK_ENABLED
            add_comments_to_log_file $DAT_FILE_SIMPLE_TIMING $ALGORITHIM_SIMPLE $BLOCK_ENABLED
...
if [ "$BUILD_BLOCKED_IJK" == "true" ]  || [ "$BUILD_ALL" == "true" ] ; then 
    ...
    if [[ ${#NX_ARRAY[*]} -ne ${#NB_ARRAY[*]} ]] ; then 
        ...
        for (( i = 0 ; i < ${#NX_ARRAY[@]} ; i++ )) do
            ...
            add_comments_to_log_file $LOG_FILE_BIJK_VALUES ${ALGORITHIM_BIJK $BLOCK_ENABLED
            add_comments_to_log_file $DAT_FILE_BIJK_TIMING $ALGORITHIM_BIJK $BLOCK_ENABLED
