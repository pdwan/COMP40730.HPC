...
void multipy_ABC_simple(double *matrix_a, double *matrix_b, double *matrix_c, int rows, int cols, FILE *fp)
{
    int ni, nj, nk;
    int count = 1;
    fprintf(fp, "\n# |C| : \tMatrix computed values for matrix of size <%d> x <%d> ... using SIMPLE : Blocked KIJ\n", rows, cols);
    for (ni=0; ni<rows; ni++)
    {
        for (nj=0; nj<cols; nj++)
        {
...
void multipy_ABC_complex(double *matrix_a, double *matrix_b, double *matrix_c, int rows, int cols, FILE *fp)
{
    int ni, nj, nk;
    int count = 1;
    fprintf(fp, "\n# |C| : \tMatrix computed values for matrix of size <%d> x <%d> ... using COMPLEX : Blocked KIJ\n", rows, cols);
    for (ni=0; ni<rows; ni++)
    {
        for (nj=0; nj<cols; nj++)
        {
...
//                 successive rows for row-major storage or columns for column-major storage. 
    ...
    fprintf(fp, "\n# |C| : \tMatrix computed values for matrix of size <%d> x <%d> ... using DEGEMM : Blocked KIJ\n", rows, cols);
