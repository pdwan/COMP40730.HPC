...
//  CREATE & INITIALIZE :: matrices A & B & C and output results to matrix file for reference
    ...
    {    
        ...
        {
            fprintf(fp_matrix, "# INITIALIZE : \t<%d> x <%d> matrix |A|  using random mod 10 ... \n", nx, ny);
            init_matrix_random(A, nx, ny);
            print_matrix(A, nx, ny, fp_matrix);
            fprintf(fp_matrix, "# INITIALIZE : \t<%d> x <%d> matrix B| using random mod 10 ... \n", nx, ny);
            init_matrix_random(B, nx, ny);
            print_matrix(B, nx, ny, fp_matrix);
      } else if (increment_or_random == 1) 
        {
            fprintf(fp_matrix, "# INITIALIZE : \t<%d> x <%d> matrix |A| using incremental <column> value + 1 ... \n", nx, ny);
            init_matrix_increment(A, nx, ny);
            print_matrix(A, nx, ny, fp_matrix);            
            fprintf(fp_matrix, "# INITIALIZE : \t<%d> x <%d> matrix B| using incremental <column> value + 1 ... \n", nx, ny);
...
//  MANUAL execution : calculate |C| and infinity norm for resulting |C|
    ...
    print_matrix(C, nx, ny, fp_matrix) ;     
    fprintf(fp_matrix, "# \t\tMatrix |C| calculated in %f seconds and has infinity norm of [%f] ... \n", manual_elapsed, manual_norm) ; 
    fprintf(stdout, "# RESULTS : \tmanual Straight-forward IJK calculation ... \n") ; 
    fprintf(stdout, "# \t\tMatrix |C| calculated in%f seconds and has infinity norm of [%f] ... \n", manual_elapsed, manual_norm) ; 
...
//  CALCULATION :: |C| using MPI
    fprintf(stdout,"# INITIALIZE : \t|C| ... \n");
    fprintf(fp_matrix, "# INITIALIZE : \t<%d> x <%d> matrix  |C| for MPI calculation ... \n", nx, ny) ; 
    init_matrix_zero(C, nx, ny) ; 
    print_matrix(C, nx, ny, fp_matrix) ; 
    ...
    gettimeofday(&tv1, &tz) ; 
    fprintf(stdout,"DEBUG :\trank = %d \n", rank);
    fprintf(stdout,"DEBUG :\tsize = %d \n\n", size);
    fprintf(stdout,"DEBUG :\tnx_proc_segment = %d \n", nx_proc_segment);
    fprintf(stdout,"DEBUG :\tnx_start = %d \n",nx_start );
    fprintf(stdout,"DEBUG :\tnx_end  = %d \n\n",nx_end );
    fprintf(stdout,"DEBUG :\tMPI_Barrier \n");
    MPI_Barrier(MPI_COMM_WORLD);
    fprintf(stdout,"DEBUG :\tMPI_Bcast\n");
    MPI_Bcast (B, nx*ny, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    fprintf(stdout,"DEBUG :\tMPI_Scatter\n");
    ...
    for (ni=nx_start; ni<nx_end; ni++)
    {
        for (nj=0; nj<ny; nj++)
        {
            for (nk=0; nk<nx; nk++)
            {
                ...
                fprintf(stdout,"nx_start = [%d], \t C[(ni*nx)+nj] = [%g] \n ", nx_start, C[(ni*nx)+nj]);
            }
        }
    }
    MPI_Barrier(MPI_COMM_WORLD);
    MPI_Gather (C+nx_start, nx_proc_segment, MPI_DOUBLE, C, nx_proc_segment, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    double mpi_norm =0.0,mpi_row_norm =0.0 ; 
    if (rank ==0 )
    {
        double mpi_row_norm =0.0, mpi_norm =0.0 ; 
        for (ni=0 ; ni<nx ; ni++)
        {
            mpi_row_norm =0.0 ; 
            for (nj=0 ; nj<nx ; nj++)
            {
                mpi_row_norm += C[(ni*nx) +nj] ; 
            }
