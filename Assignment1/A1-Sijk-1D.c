...
//  CALCULATION :: |C| using manual complex
    fprintf(stdout,"# RESULTS : complex manual calculation for complex manual Straight-forward IJK ... \n");
    fprintf(fp_matrix, "# Initialize matrix <%d> x <%d> |C| for complex manual Straight-forward IJK .. \n", nx, ny);
    init_matrix_zero(C, nx, ny);
    print_matrix(C, nx, ny, fp_matrix);
    gettimeofday(&tv1, &tz);
    for (ni=0; ni<nx; ni++)
    ...
    fprintf(fp_matrix, "# |C| : <%d> x <%d> matrix computed values for complex manual Straight-forward IJK ... \n", nx, ny);
    print_matrix(C, nx, ny, fp_matrix);
    fprintf(fp_matrix, "# |C| : matrix calculated in %f seconds ... \n", complex_elapsed);
