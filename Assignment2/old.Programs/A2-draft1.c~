...
// Get values for matrix row x col matrix, both equal to n
   printf("    Please enter matrix dimension < maximun = %d --> n : ", MAXNMATRIX);
   int nx = get_value(MAXNMATRIX);
   int ny = nx;

// Get number of processes
   printf("    Please enter number of processors (i.e.: number of threads) < %d --> <np>: ", MAXPTHREADS);
...
// Commence pthread calculation
   ...
   double *norm_summation_matrix = malloc(ns_size * (sizeof(double)));
   void *status; 
