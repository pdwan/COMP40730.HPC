/* 
[pdwan@csserver Assignment2]$ gcc  -I/home/cs/khasanov/libs/CBLAS/src A2-pthreads-1D.c -o A2-pthreads-1D-cblas  /home/cs/khasanov/libs/cblas_LINUX.a  /usr/lib/libblas.a -lgfortran -pthread
A2-pthreads-1D.c: In function ‘calc_row_norm’:
A2-pthreads-1D.c:190: error: incompatible types in return
A2-pthreads-1D.c: In function ‘multiply_abc_pthreads’:
A2-pthreads-1D.c:270: warning: assignment from incompatible pointer type
A2-pthreads-1D.c:285: warning: passing argument 3 of ‘pthread_create’ from incompatible pointer type
[pdwan@csserver Assignment2]$ 
...
double *calc_row_norm(void *args)
{
     norm_sum_t *norm_sum_data ; 
     int i ;  
     int ALPHA=1.0 ; 
     int BETA=0.0 ; 
     double l_norm_sum =0.0;
     ...
     {
          norm_sum_data->my_norm_sum[counter] += norm_sum_data->my_sliceC[i] ; 
          ...
          if (norm_sum_data->my_norm_sumC < norm_sum_data->my_norm_sum[i]) 
          {
               norm_sum_data->my_norm_sumC = norm_sum_data->my_norm_sum[i] ; 
          }
     fprintf(stdout, "# DEBUG : \t%g \t [%g] \n",  (double)norm_sum_data->my_norm_sum[i], norm_sum_data->my_norm_sumC) ; 
     }  
     
     fprintf(stdout, "# DEBUG :  Max absolute value in local |sliceC| is : %g\n", norm_sum_data->my_norm_sumC) ;     
     l_norm_sum = norm_sum_data->my_norm_sumC ;
...
double multiply_abc_pthreads (double *all_a, double *all_b, double *all_c, int row, int thd, int sli) 
{

    pthread_t *working_thread = malloc(sli * (sizeof(pthread_t) )) ; 
    pthread_mutex_t *mutex_dot_product = malloc((sizeof(pthread_mutex_t))) ; 
    norm_sum_t *thread_norm_sum_data = malloc(thd * (sizeof(norm_sum_t ))) ; 
    double *norm_sum_matrix = malloc( sli * (sizeof(double))) ; 
    void *status ; 
    ...
    for(i=0 ; i<thd ; i++) 
    { 
        fprintf(stdout, "# DEBUG :\tCreate working thread [%d] and send to calc_row_norm function ...\n", i) ; 
        thread_norm_sum_data[i].my_sliceA = all_a + (i * sli) ; 
        thread_norm_sum_data[i].my_sliceC = all_c + (i * sli) ; 
        thread_norm_sum_data[i].my_allB = all_b ; 
        thread_norm_sum_data[i].my_norm_sum = &norm_sum_matrix ; 
        thread_norm_sum_data[i].my_norm_sumC = -1.0 ; 
        thread_norm_sum_data[i].mutex = mutex_dot_product ; 
        thread_norm_sum_data[i].my_no_of_slices = sli ; 
        thread_norm_sum_data[i].my_nx = row ; 
        thread_norm_sum_data[i].my_np = thd ;       
        // double *my_sliceA ; 
        // double *my_sliceC ; 
        // double *my_allB ; 
        // double *my_norm_sum ; 
        ...
        // int my_nx ; 
        // int my_np ; 
        pthread_create(&working_thread[i], NULL, calc_row_norm, (void*)&thread_norm_sum_data[i]) ; 
    
        double pthread_norm = thread_norm_sum_data[i].my_norm_sumC ; 
        fprintf(stdout, "# DEBUG :\tbefore if - pthread_norm = [%g] \n", pthread_norm ) ; 
        fprintf(stdout, "# DEBUG :\tbefore if - inf_norm = [%g]\n", inf_norm) ;    
        pthread_norm = ( pthread_norm < inf_norm ) ? inf_norm : pthread_norm ; 
        fprintf(stdout, "# DEBUG :\tafter if - pthread_norm = [%g]\n", pthread_norm ) ;    
        fprintf(stdout, "# DEBUG :\tafter if - inf_norm = [%g]\n", inf_norm) ;    
    } 
...
//  MANUAL execution : calculate |allC| and infinity norm for resulting |C|
    ...
    print_matrix(allC, nx*ny, stdout) ; 
...
//  PTHREAD execution : Commence pthread calculation
    fprintf(fp_matrix, "# Initialize matrix [%d] x [%d] |C| for pthread IJK BLAS/ATLAS computation ... \n", nx, ny) ; 
    init_matrix_zero(allC, nx*ny) ; 
    print_matrix(allC, nx*ny, fp_matrix) ; 
    gettimeofday(&tv1, &tz) ; 
    fprintf(stdout, "# DEBUG : \tCalculate infinity norm of |allB| and |allA| using |sliceA| writing to |sliceC|...\n") ; 
    gettimeofday(&tv1, &tz) ; 
    double pthread_norm = multiply_abc_pthreads(allA, allB, allC, nx, np, no_of_slices) ; 
    gettimeofday(&tv2, &tz) ; 
...
//  OUTPUT :: results to stdout & .dat file : matrix size || no of processors ||infinity norm || Time/manual || infinity norm || Time / dgemm
    ...
    fprintf(stdout, "# Results: \t%d \t%d \t%f \t%g \t%f \t%g \t%f \t%g \n", nx, np, manual_elapsed, manual_norm, dgemm_elapsed, dgemm_norm, pthread_elapsed, pthread_norm) ; 
