...
void usage () 
{
    ...
    fprintf(stdout,"\nTO :\tCalculate |C| = |A| x |B| manually for MPI comparison and also calculate infinity norm of |C|. \n");    
...
void initialize_data_file_contents (FILE *fp) 
{
    ...
    fprintf(fp, "# Program :\tA4-mpi-1D-manual \n# where :\t.dat contains timing data & .txt contains matrix values \n# \n");
    fprintf(fp, "# --------------------------------------------------------------------------------------------------\n# \n");
    fprintf(fp, "# |Matrix| \t|Processors| \tTime/manual \tInf Norm/manual \tTime/dgemm \tInf Norm/dgemm \n# \n");
}
...
//  OUTPUT :: results to stdout & .dat file : |Matrix| || |Processors| ||inf norm/manual || Time/manual || infinity norm || Time / dgemm
    fprintf(stdout,"#SUMMARY :\t|Matrix|  |Processors| Time/manual Inf Norm/manual Time/dgemm \tInf Norm/dgemm \n");
